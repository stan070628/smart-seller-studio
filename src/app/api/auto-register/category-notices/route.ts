...
async function generateNoticesWithAI(
  ...
  const descSection = productDesc
    ? `\n[상품 스펙 정보 — 아래 값들을 고시정보 작성에 직접 활용하세요]\n${productDesc}\n`
    : '';

  const userPrompt = `상품명: ${productName}${descSection}
...
1. 위 목록에서 이 상품에 가장 적합한 고시정보 카테고리를 1개만 선택하세요.
   ...
   - 실제 의류(티셔츠/바지/원피스/점퍼 등 옷) → 목록에서 의류/섬유 관련 카테고리
   - 실제 신발 → 목록에서 신발 카테고리
   - 실제 가방/지갑/배낭 → 목록에서 가방 카테고리
   - 실제 모자/벨트/스카프/넥타이/장갑 등 패션잡화 → "패션잡화(모자/벨트/액세서리 등)"
   ※ 전자기기 액세서리(케이블·케이스·거치대·충전기 등)는 절대 패션잡화가 아님 → "기타 재화"
   ※ 생활용품·주방용품·스포츠용품·완구·식품·문구·펫 용품 등은 → "기타 재화"
   - 위 기준에 맞지 않거나 해당 카테고리가 목록에 없을 때 → "기타 재화"
   ※ 반드시 위 [목록]에 있는 카테고리명 중에서만 선택하세요. 불확실하면 "기타 재화"로 선택하세요.
...
- 원산지, 품명, 소재, 크기, 무게 등 기본 스펙 항목에는 절대 "상품상세 참조" 사용 금지
- 색상, 치수, 소재, 제조국, 제조연월 등은 "상품 스펙 정보"에 해당 정보가 있으면 반드시 그 값을 그대로 사용
- 상품 스펙 정보에서 찾을 수 없는 경우에만 "확인 필요"로 작성 (절대로 추측하지 말 것)
- 제조연월: 상품 스펙 정보에 없으면 반드시 "확인 필요"로 작성 (날짜 추측 금지)
