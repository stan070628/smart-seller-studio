...
export async function POST(
  ...
  try {
    ...
    // vendorItemId → 옵션명 매핑 구성
    const variants: Record<string, string> = {};
    for (const item of items) {
      const vendorItemId = String(item.vendorItemId ?? '');
      if (!vendorItemId) continue;

      // attributes 배열에서 사이즈/색상 속성 우선 추출, 없으면 itemName, 그것도 없으면 vendorItemId 자체 사용
      ...
      const variantName =
        (sizeAttr ? String(sizeAttr.attributeValueName ?? '') : '') ||
        String(item.itemName ?? item.vendorItemName ?? '') ||
        vendorItemId;
      variants[vendorItemId] = variantName;
    }
    ...
    const isNotFound = msg.includes('not found') || msg.includes('400') || msg.includes('404');
    const userMsg = isNotFound
      ? `등록상품 ID(${sellerProductId})를 찾을 수 없습니다. ✏️ 버튼으로 판매자상품ID를 확인해주세요. (vendorItemId가 아닌 등록상품ID를 입력해야 합니다)`
      : msg;
