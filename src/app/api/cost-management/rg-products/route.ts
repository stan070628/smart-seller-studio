...
export async function GET() {
  try {
    ...
    // ── Phase 3: Wing RG 상품 목록으로 이름 보완 ────────────────────────────
    // externalSkuId는 판매자 정의 SKU ID로 Wing sellerProductId와 무관(검증됨).
    // seller-products?businessTypes=rocketGrowth API로 vendorItemId→상품명 맵을 구성.
    const unnamedCount = [...inventoryIds].filter((vid) => !nameMap.has(vid)).length;
    if (unnamedCount > 0) {
      const wingNameMap = await client.getRocketGrowthProductNames();
      for (const [vendorItemId, name] of wingNameMap) {
        if (!nameMap.has(vendorItemId)) nameMap.set(vendorItemId, name);
      }
    }
