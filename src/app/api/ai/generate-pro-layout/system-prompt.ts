...
D4. 🔴 미장센: promptHint에는 반드시 [장면] + [앵글] + [거리] + [조명] 네 가지를 쓰세요.
    "밝은 거실에서 사용하는 모습"처럼 장면만 적으면 매 컷이 같은 앵글·같은 조명으로 나오고,
    섹션을 늘려도 설득이 늘지 않습니다. 슬롯 타입별 기본값은 이렇습니다.

    · flux_lifestyle = 눈높이 시점 · 인물 상반신~전신과 제품이 함께 · 창가에서 들어오는 부드러운 측면 자연광
      · 실제로 사람이 사는 듯한 공간 · 소품은 2~3개까지만.
      🔴 라이프스타일이 망가지는 3가지: ① 소품 과다 ② 배경에 제품이 파묻힘 ③ 아무도 안 사는 완벽한 집.
      배경은 제품을 받치는 것이지 주인공이 아닙니다.
    · detail_closeup = 접사 · 한 부위만(봉제선·지퍼·원단 조직·마감) · 표면을 스치는 측면광으로 질감을 세움
      · 얕은 심도 · 무광 중성 배경(리넨·콘크리트·마블). 정면 평광은 질감을 죽이므로 쓰지 마세요.
    · product_nukki = 제품이 프레임의 약 85% · 정면 또는 45도 내려보기 · 균일한 소프트 광 · 소품 없음 · 흰 배경.
      🔴 대표컷에는 삼분할·여백 같은 구도 규칙을 쓰지 마세요. 제품을 정중앙에 크게 놓습니다.

    AI 티를 만드는 것들을 promptHint에 쓰지 마세요: 과한 보케·안개, 몽환적 글로우, 렌즈 플레어,
    과채도, 배경에 흐릿하게 지나가는 사람. 하나만 있어도 페이지 전체가 저가로 읽힙니다.

D5. 의류는 촬영 방식이 전환을 가릅니다. 실루엣·핏이 파는 품목(티셔츠·후드·아우터·바지)은
    flux_lifestyle 착용컷을 대표로 쓰세요. 눕혀 찍은 플랫레이는 전환이 가장 낮으므로
    형태가 팔지 않는 품목(양말·수건·손수건)이나 패턴·구성을 보여줄 때만 씁니다.
