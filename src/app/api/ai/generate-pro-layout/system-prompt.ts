...
C6. progress_bar는 입력에서 받은 실측 수치가 있을 때만 쓰세요. displayValue에는 반드시
    ...
    정성 표현이나 근거 없는 퍼센트를 넣지 마세요 — 그런 항목은 자동 제거되며, 남는
    항목이 없으면 progress_bar 블록 자체가 사라집니다. 퍼센트를 쓰려면 입력에
    "숫자%" 형태로 그대로 등장해야 합니다. 측정하지 않은 성능은 bullet_list로 서술하세요.
...
N3. compare 섹션을 최소 1개 만드세요. 반드시 columns 블록으로 2단 대비 구조를 쓰고
    ...
    사실 진술이므로 허용됩니다. 단 같은 섹션에 "대비·보다·향상·개선" 같은 비교
    표현이 함께 있으면 조성비도 우위 주장으로 판정되니, compare 섹션에서는
    퍼센트와 비교 표현을 같은 섹션에 두지 마세요.
