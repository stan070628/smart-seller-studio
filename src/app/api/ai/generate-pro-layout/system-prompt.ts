...
N7. 인물 착용컷(model_wearing) — 착용이 구매를 좌우하고 제품이 상반신(가슴~엉덩이)
    ...
    - faceVisible을 반드시 명시하세요 — 하나는 true(얼굴 컷), 하나는 false(크롭 컷).
      생략하면 얼굴 컷이 되어 두 컷이 같아집니다(얼굴 컷=구매욕 유발, 크롭 컷=핏·마감 확인).
    ...
    - promptHint에는 [상황]만 쓰세요(예: "여름 해변 보드워크 산책") — 모델 외형·
      프레이밍·조명·포즈는 시스템이 붙이므로 겹쳐 쓰면 충돌하고, 두 씬의 상황도 겹치면 안 됩니다.
    ...
    - problem 비트에는 인물을 쓰지 마세요. D3의 비교 대상 예외도 problem에는 적용되지
      않습니다.
    - model_wearing 섹션에는 product_nukki도 넣어(model_wearing 첫 번째) image 블록 2개로
      병치하세요: 첫째 attachedIndex 0(착용컷), 둘째 attachedIndex 1·"width":"45%"(제품 단독컷).
