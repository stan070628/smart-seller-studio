...
3. 혜택/특장점을 포함 (무료배송, 대용량, 1+1 등)
4. 50자 내외 (너무 길면 잘림)
5. 특수문자 최소화, 가독성 우선
6. 도매꾹/공급사/도매 관련 단어 절대 포함 금지
...
function buildUserPrompt(title: string, category?: string): string {
  let prompt = `원본 상품명: ${title}`;
  if (category) prompt += `\n카테고리: ${category}`;
  prompt += `\n\n위 도매꾹 상품명을 쿠팡/네이버용으로 최적화하고, 검색 태그를 생성해주세요.`;
  return prompt;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { originalTitle, categoryName } = RequestSchema.parse(body);
    ...
    const response = await withRetry(
      () =>
        client.messages.create({
          ...
          messages: [{ role: 'user', content: buildUserPrompt(originalTitle, categoryName) }],
        }),
