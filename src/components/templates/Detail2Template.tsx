...
const Detail2Template: React.FC<TemplateProps> = ({
  ...
  const displayPoints = bulletPoints.length > 0 ? bulletPoints
    : ['세련된 미니멀 디자인', '다양한 공간에 어울리는 컬러', '선물로도 손색 없는 프리미엄 패키지'];
  ...
  return (
    <div style={{
      ...
      <div style={{ height: '45%', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
        {imageUrl ? (
          ...
          <img src={imageUrl} alt="상품 이미지"
            style={{ width: '100%', height: '100%', objectFit: imageFit, objectPosition: 'right center', transform: `scale(${imageScale})`, transformOrigin: `${imageOffsetX}% ${imageOffsetY}%`, display: 'block' }}
          />
        ) : (
        ...
        {imageUrl && (
          <div style={{
            ...
            background: `linear-gradient(to bottom, transparent, ${theme.bgSubtle})`,
            pointerEvents: 'none',
          }} />
        )}
      </div>
      ...
      <div style={{
        ...
        <EditableText value={frame.headline || '감성적인 디자인'} field="headline"
        ...
        {frame.bodyText && (
          <EditableText value={frame.bodyText} field="bodyText"
            isEditable={isEditable} onFieldChange={onFieldChange as ((f: string, v: string) => void) | undefined}
            tag="p"
            style={{ color: theme.accentText, fontSize: '14px', lineHeight: '1.7', marginTop: '24px', padding: '18px', backgroundColor: theme.bgAccentLight, borderRadius: '10px', display: 'block' }}
          />
        )}
      </div>
    </div>
