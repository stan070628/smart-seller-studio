...
describe('buildVerifyResult — 수동 입력 p25 사용', () => {
  it('저장된 p25로 판정한다 (외부 시세 조회 없음)', async () => {
    const r = await buildVerifyResult(dome, 10, 'xsmall', 14000);

    expect(r.coupangP25).toBe(14000);
    expect(r.effectiveCost).toBe(3600);    // 3300 + ceil(3000/10)
    expect(r.breakEvenPrice).toBe(13046);  // 2026-08-13 실청구 물류비 + 매출세액 반영
    expect(r.verdict).toBe('pass');        // 14000 >= 13046
  });
  ...
  it('p25가 null이면 unknown이되 원가·손익분기는 채운다', async () => {
    ...
    expect(r.breakEvenPrice).toBe(13046);  // 시세를 몰라도 손익분기는 계산된다
