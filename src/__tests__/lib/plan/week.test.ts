...
describe('getWeekForDate', () => {
  it('PLAN_START 당일은 Week 1을 반환한다', () => {
    expect(getWeekForDate('2026-04-27')).toBe(1);
  });

  it('PLAN_START + 6일은 Week 1을 반환한다', () => {
    expect(getWeekForDate('2026-05-03')).toBe(1);
  });

  it('PLAN_START + 7일은 Week 2를 반환한다', () => {
    expect(getWeekForDate('2026-05-04')).toBe(2);
  });
...
describe('getCurrentWeek', () => {
  ...
  it('현재 시점 기준 주차를 반환한다 (PLAN_START + 4일 → Week 1)', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-05-01T10:00:00+09:00'));
    expect(getCurrentWeek()).toBe(1);
  });

  it('PLAN_START + 14일 → Week 3', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-05-11T10:00:00+09:00'));
...
describe('getDaysIntoWeek', () => {
  ...
  it('PLAN_START 당일은 1을 반환한다', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-04-27T10:00:00+09:00'));
  ...
  it('주의 마지막 날(7일째)은 7을 반환한다', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-05-03T10:00:00+09:00'));
    expect(getDaysIntoWeek()).toBe(7);
  });

  it('새 주의 첫날은 다시 1로 돌아온다', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-05-04T10:00:00+09:00'));
    expect(getDaysIntoWeek()).toBe(1);
  });
});
