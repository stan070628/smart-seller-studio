...
export function buildDetailPageUserPrompt(
  ...
  if (productSpecs && productSpecs.length > 0) {
    ...
    // 고시정보를 스펙으로 넘긴 경우, 그 값들은 제조사가 표기한 확정 사실이다.
    // 사진 추론으로 덮어쓰거나 바꿔 쓰면 실제 표기와 어긋난다(핸드워시 성분 오기 사례).
    const hasNotice = productSpecs.some(s => /사용법|주요 성분|제조업자|책임판매업자|제조국|사용기한/.test(s.label));
    if (hasNotice) {
      lines.push('- 위 항목 중 사용법·성분·제조사·사용기한은 **제조사가 표기한 확정 사실**입니다. 문장을 다듬는 것은 되지만 내용을 바꾸거나 없는 성분·효능을 더하지 마세요.');
      lines.push('- 성분은 위 목록에 있는 것만 언급하세요. 목록에 없는 성분명을 주요 성분으로 쓰면 표시 위반입니다.');
      lines.push('- 사용법은 제조사 표기를 그대로 살리세요. 임의로 단계 수나 시간을 바꾸지 마세요.');
      lines.push('- 임상 수치·효능 퍼센트는 위 스펙에 없으면 절대 쓰지 마세요(실증자료 보유 의무).');
    }
  }
