...
describe('computeDailySettlement', () => {
  it('매출은 sale_amount 우선, 수수료는 (매출-쿠폰)×수수료율 건별 반올림', () => {
    const sales = [
      { sold_at: '2026-07-16', sale_amount: 30000, selling_price: 30000, quantity: 2, coupon_discount: 0, platform_fee_rate: 0.1 },
    ];
    ...
    expect(r.parcelFee).toBe(0);          // 택배비는 이제 판매에서 안 나옴(수동 비용에서만)
    expect(r.netProfit).toBe(27000);       // 30000 - 3000
  });

  it('sale_amount 없으면 selling_price × quantity 폴백', () => {
    const sales = [
      { sold_at: '2026-07-16', sale_amount: null, selling_price: 20000, quantity: 3, coupon_discount: 0, platform_fee_rate: 0.1 },
    ];
    const { rows } = computeDailySettlement(sales, noEntries, noExpenses);
  ...
  it('쿠폰은 수수료 계산 전 차감 (건별)', () => {
    const sales = [
      { sold_at: '2026-07-16', sale_amount: 40000, selling_price: 40000, quantity: 2, coupon_discount: 6000, platform_fee_rate: 0.1 },
    ];
    const { rows } = computeDailySettlement(sales, noEntries, noExpenses);
  ...
  it('상품별 수수료율이 다르면 건별 계산 후 합산', () => {
    const sales = [
      { sold_at: '2026-07-16', sale_amount: 10000, selling_price: 10000, quantity: 1, coupon_discount: 0, platform_fee_rate: 0.108 },
      { sold_at: '2026-07-16', sale_amount: 10000, selling_price: 10000, quantity: 1, coupon_discount: 0, platform_fee_rate: 0.05 },
    ];
    const { rows } = computeDailySettlement(sales, noEntries, noExpenses);
  ...
  it('수동 비용: 택배비·광고비·박스비 차감', () => {
    const expenses = [
      { expense_date: '2026-07-16', ad_spend: 85000, box_cost: 120000, parcel_cost: 42000 },
    ];
    const sales = [
      { sold_at: '2026-07-16', sale_amount: 300000, selling_price: 300000, quantity: 1, coupon_discount: 0, platform_fee_rate: 0 },
    ];
    ...
    expect(rows[0].parcelFee).toBe(42000);   // 택배비 = parcel_cost(수동)
    // 300000 - 0 - 0 - 0 - 42000 - 85000 - 120000 = 53000
    expect(rows[0].netProfit).toBe(53000);
  });

  it('택배비 수동 입력만 있는 날(판매 없음) → 음수 순이익', () => {
    const expenses = [
      { expense_date: '2026-07-16', ad_spend: 0, box_cost: 0, parcel_cost: 30000 },
    ];
    const { rows } = computeDailySettlement([], noEntries, expenses);
    expect(rows[0].parcelFee).toBe(30000);
    expect(rows[0].netProfit).toBe(-30000);
  });

  it('여러 날짜를 날짜 내림차순으로, monthTotal은 전체 합산', () => {
    const sales = [
      { sold_at: '2026-07-15', sale_amount: 10000, selling_price: 10000, quantity: 1, coupon_discount: 0, platform_fee_rate: 0 },
      { sold_at: '2026-07-16', sale_amount: 20000, selling_price: 20000, quantity: 1, coupon_discount: 0, platform_fee_rate: 0 },
    ];
