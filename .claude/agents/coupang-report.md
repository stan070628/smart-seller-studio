...
`$B pdf`는 HTML 경로를 받아 동일 디렉토리에 .pdf 파일을 생성한다.
`$B pdf`가 실패하면 오류 메시지를 확인하고, 경로에 한글이나 공백이 있는지 점검한다.
PATH에 한글이 있어서 문제가 생기면 `/tmp/coupang-report.html`처럼 영문 경로로 시도한다.

완료 후 사용자에게 다음 형식으로 보고한다:
...
━━━━━━━━━━━━━━━━━━━━━━━━━
핵심 요약:
- 노출: [ADS_IMPRESSIONS]회 | CTR: [ADS_CTR]% ([CTR 평가: 위험/주의/보통/우수])
- 주문: [ADS_ORDERS]건 | ROAS: [ADS_ROAS]%
- 최우선 액션: [긴급 액션 1순위]
```
