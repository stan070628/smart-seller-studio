...
**결론: 검증한 후보를 담아둘 곳이 없고, 담아도 낡는다.** 후보는 채팅 로그에만 남았다.
...
**`breakEvenPrice(cost, logisticsSize)`**

두 조건을 모두 만족하는 최소 판매가. 큰 쪽을 취하고 **올림**한다.

- 마진율 30% 이상 → `(원가 + 물류비) / (1 − 0.108 − 0.30)`
- 개당 마진 ≥ 물류비 × 1.5 → `(원가 + 물류비 × 2.5) / (1 − 0.108)`
...
| `PATCH /api/sourcing/shortlist/[itemNo]` | memo·logistics_size 수정 (수정 시 손익분기 재계산) |
...
소싱 리스트                                    [+ 추가]  [전체 재검증]
────────────────────────────────────────────────────────────────
상품              도매가   쿠팡p25  손익분기  마진율  사이즈   상태
메쉬 반장갑        3,300    9,900    8,535    38%   극소형▾  ✅
대형 무릎담요      3,180   10,200    8,891    39%   소형▾    ✅
접이식 쓰레기통    2,530    5,080    7,671     5%   극소형▾  ❌
캠핑 그릴 수납가방  4,500       —        —      —   극소형▾  ⚠ 판매종료
반려동물 유골함    3,500       —        —      —   극소형▾  ⚠ 표본부족
```
...
- 쇼트리스트에 배송비(`deli_fee`)를 반영할 것인가. 현재 `sourcing_items`의 `deli_fee`는 45만 건 중 2,539건(0.6%)만 채워져 있어 실제 원가가 과소 계상된다. 도매꾹 `getItemView`의 `deli` 필드를 검증 시점에 함께 저장하면 해결되나, `deli-parser.ts`의 파싱 정확도를 먼저 확인해야 한다.
