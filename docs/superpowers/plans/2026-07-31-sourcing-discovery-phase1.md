...
**Tech Stack:** Next.js App Router, TypeScript, Vitest, Postgres(`getSourcingPool`), 도매꾹 API, ~~네이버 쇼핑 API~~ (2026-07-31 종료 — 아래 참조)

**Spec:** `docs/superpowers/specs/2026-07-31-sourcing-discovery-pipeline-design.md`

---

## ⚠️ 2026-07-31 — 이 계획의 전제 하나가 무너졌다

**네이버 쇼핑 검색 API가 2026-07-31자로 영구 종료됐다.** 이 계획을 전부 구현한 당일이다.

> 네이버 개발자센터 공지 32530:
> 검색 API 중 **'쇼핑·책·전문자료' 검색 API는 NAVER API HUB 이관 대상에서 제외**되며,
> 단계별 일정과 관계없이 **2026년 7월 31일에 서비스가 종료**된다.
> **유예 기간이 적용되지 않으며**, 이후에는 **기존 발급 키를 포함하여 호출이 불가**하다.
> **NAVER API HUB를 포함한 별도의 대체 API는 제공되지 않는다.**

### 확인한 증거

| 검증 | 결과 |
|---|---|
| `GET /v1/search/shop.json` | **HTTP 404 · errorCode `SE05`** (존재하지 않는 검색 api) |
| `GET /v1/search/blog.json` (같은 키) | **HTTP 200 OK** — 자격증명은 정상, 쇼핑만 폐기 |
| `costco_market_prices` 중 `source='naver_api'` | 마지막 기록 **2026-04-11**, 이후 0건 |
| `sourcing_shortlist` 중 `coupang_p25 IS NOT NULL` | **0건 — 한 번도 채워진 적 없음** |

마지막 줄이 핵심이다. Task 7이 판정 기준으로 삼은 쿠팡 p25는 **운영에서 단 한 번도 산출된 적이 없다.**
코스트코 최저가 비교가 동작하는 것처럼 보이는 이유는 DB에 4월 데이터가 남아 있어서다 — 화면은 뜨지만 값은 3개월 반 전 것이다.

### 죽은 것과 살아남은 것

| 죽음 | 근거 |
|---|---|
| `estimateCoupangPrice` (`coupang-price.ts`) | 쿠팡 p25 — **Task 7 판정의 기준** |
| `searchNaverLowestPrice` · `searchNaverUnitPrice` (`naver-shopping.ts`) | |
| 코스트코 최저가 비교, `niche/naver-shopping` 쇼핑 검색 | |

| 생존 | |
|---|---|
| Task 1·2·3·4 — 원가 모델 | 수수료 VAT 11.88%, 물류비 실효원가, 관세 과세운임, 해운 요율. **외부 API를 쓰지 않는다** |
| Task 5·6 — 시드 발굴·조회 | 시드 수집은 Gemini, 조회는 자체 DB |
| Task 8·9·10 — 적재·실행 라우트·발굴 탭 | |

**`breakEvenPrice(실효원가, 사이즈)`는 순수 함수라 그대로 동작한다.** 판정의 어려운 절반(원가·수수료·물류비·관세)은 살아 있고, 없어진 것은 "쿠팡에서 실제로 얼마에 팔리나" 한 값뿐이다.

### 대안 조사 결과

| 후보 | 판정 |
|---|---|
| 쿠팡 윙 OPEN API (`COUPANG_ACCESS_KEY` 등, 클라이언트 기구현) | ❌ 자사 상품 관리용. 경쟁사 시세 조회 불가 |
| `NAVER_COMMERCE_*` | ❌ 자사 스마트스토어 관리용 |
| `NAVER_AD_*` (검색광고) | △ 검색량만. 가격 없음 |
| 네이버 쇼핑 인사이트 (NAVER API HUB로 이관) | △ 트렌드만. 가격 없음 |
| 쿠팡 파트너스 API | ○ 쿠팡 상품·가격 직접 반환. **다만 2026-07-31 기준 계정에 API 접근 권한 없음** |

### 다음 단계 — 사람이 한 칸만 채우는 구조

자동 시세를 되찾을 방법이 당장 없으므로, **기계가 못 하게 된 한 가지만 사람에게 넘긴다.**

```
자동   도매꾹 후보 발굴 · URL · 도매가
자동   실효원가 = 도매가 + 개당 배송비
자동   손익분기가  ← 오늘 고친 원가 모델 전부가 여기로 들어간다
──────────────────────────────────────
수동   쿠팡 실판가 한 칸 입력  (검색창에서 10초)
자동   pass/fail 판정 · 마진율 · 소싱리스트 정렬
```

붙일 것 셋:

1. 소싱리스트에 **쿠팡가 입력 칸** — `PATCH /api/sourcing/shortlist/[itemNo]`가 현재 허용하는 필드는
   `memo`·`logisticsSize`·`orderQty`·`isArchived`뿐이다. `coupangP25`를 추가하고, 입력 시 판정을 재계산한다.
2. **죽은 자동 조회가 수동 입력값을 덮지 않게 차단** — 지금은 `logisticsSize`/`orderQty`를 바꾸면
   `verifyOne`이 돌아 `unknown`으로 되돌린다.
3. 분석결과 탭에 **손익분기가 표시** — 현재는 마진율만 있고 그 값이 항상 `null`이라 빈칸이다.

아울러 `shortlist-verify` 크론(매일 01:00 KST)은 시세를 못 구하므로 재검증 의미가 사라졌다.
도매꾹 재고·가격 갱신은 여전히 가치가 있으니 **시세 부분만 떼고 유지**한다.

> 이 대응은 별도 계획으로 다룬다. 규모는 이 계획의 1/5 정도다.
...
- [ ] ~~`/sourcing`에서 시드를 골라 실행하면 소싱리스트에 `pass` 후보가 쌓인다~~
      **달성 불가 (2026-07-31).** 네이버 쇼핑 검색 API 종료로 쿠팡 p25를 구할 수 없어
      모든 후보가 `unknown` 판정이 된다. 후보 발굴·URL·실효원가·손익분기가까지는 정상 동작한다.
      수동 시세 입력을 붙이는 후속 계획에서 재정의한다.
