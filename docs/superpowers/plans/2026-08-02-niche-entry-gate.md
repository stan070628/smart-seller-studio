...
> 🔴 **2026-08-02 중단 — 이 계획은 실행하지 않는다.**
>
> 니치 발굴은 **구매대행**을 위해 만들어진 시스템이다. 구매대행은 재고를 들지 않고
> 주문 시 사서 보내므로 대형·고가·업소용이 오히려 적합하며, "로켓 미진입 = 기회"도
> 그 모델에서는 타당한 논리였다. **시스템이 잘못 설계된 것이 아니라, 사업 모델이
> 로켓그로스 사입으로 옮겨갔는데 시스템은 옛 목적 그대로 돌고 있었다.**
>
> 소유자가 구매대행을 접기로 결정했다 — 업소용 설비는 설치·AS 문의가 따라오는데
> 셀러 경력 4개월차로는 CS를 감당할 수 없다는 판단이다.
>
> 조치: `vercel.json`에서 `/api/niche/cron`을 제거해 일일 수집을 멈췄다.
> `niche_*` 테이블 8종과 438건은 **삭제하지 않는다** — 구매대행을 재개하면 그대로 쓴다.
>
> 이 문서는 실행 계획이 아니라 **왜 멈췄는지의 기록**으로 남긴다.
