...
- **`CLAIM_MARKERS` 레지스트리** — `CLOSEUP_MARKERS`/`WEARING_MARKERS` + `isClaim` 클로저는 옳은 seam이지만 세 번째 종류에서 무너진다. `compare_pair`가 착수되면 `{ wearing?, comparePair? }` 두 boolean과 3항 OR 체인, 호출부에서 `slotType`으로 유도하는 boolean 셋, 경고 문구의 세 번째 낡은 명사가 생긴다. **세 번째 boolean을 추가하는 대신** `CLAIM_MARKERS: Record<ClaimKind, RegExp>` + `stripPhotoClaims(blocks, kinds)`로 바꾸고, `slotType → ClaimKind[]` 매핑을 `gen-slots.ts`(슬롯 타입 의미를 이미 소유한 모듈)에 두는 것이 맞다. `removed`도 `Record<ClaimKind, number>`가 되어 경고 문구가 종류별로 정확해진다. 함수·타입 이름(`stripCloseupClaims`/`CloseupStripResult`)도 그때 함께 바꾼다
- **`buildAiDisclosureSection` 추출** — 고지 객체가 `page.tsx`의 긴 함수 안에 20줄 인라인으로 있고, `[...detailSections, {…}]`가 union 원소 타입을 추론하므로 `.map()` shape에 필드가 추가돼도 **컴파일 오류가 나지 않는다**(소비자는 `as DetailSection[]`로 단언). 명시적 반환 타입 `DetailSection`을 가진 헬퍼로 빼면 드리프트 검사가 무료로 생기고 `as const` 4개와 `: undefined` 필러 2개가 사라지며 단위 테스트가 가능해진다(고지 누락 뮤테이션이 현재 잡히지 않는다). 단 배열 전체를 annotate하면 선재 느슨함 두 개가 드러난다 — `bgStyle`이 `unknown`이고 `imageSlots`가 `ClaudeLayoutContent`의 필드가 아니다. 그것은 별도 정리다
- **병치가 실제로는 수직 스택이다** — image 블록이 각각 자기 `display:flex` div로 렌더되므로 전폭 착용컷 아래 45% 제품컷이 중앙 배치된다. 나란히가 아니고, 둘이 한 화면에 보이는지는 착용컷 종횡비에 달렸다. 더 중요한 것은 **어느 쪽이 실물인지 라벨이 없다는 것** — 하단 고지는 "일부 이미지"라고만 말한다. `columns` 블록을 쓰면 인접이 보장되나 각 185px로 작아지고, 제품컷 아래 `subtext "실제 제품 사진"` 한 줄이 목적의 대부분을 달성한다. 둘 다 `N7` 줄 예산을 쓴다
- **`option_grid`와 wearing 섹션의 상호작용** — wearing 섹션이 이미지 2장을 갖게 되면서 (a) `findReusedImages`가 `urls.length === 1`인 섹션만 세므로 **wearing 섹션의 이미지 재탕이 더 이상 보고되지 않고**, (b) 2항목 `option_grid`가 있으면 `isOptionGridCarrier` 조건을 만족해 **image 블록 둘 다 제거되고** AI 착용컷이 작은 카드로 렌더된다. `N7`이 `flux_lifestyle`·`detail_closeup` 공존은 금지하지만 `option_grid`는 언급하지 않는다
