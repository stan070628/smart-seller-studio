...
브라우저에서 자동등록 페이지 진입 → 카테고리 코드 `78780` 입력 → 800ms 대기 → 다음을 확인 (78780의 실제 fullPath = `자동차용품/세차/관리용품/관리용품/광택/케미컬/유리발수코팅제`):
1. fullPath가 `자동차용품/...` 으로 표시됨
2. 수수료가 **10.8% (✓ 자동차용품)** 로 표시됨 (이전엔 정규식이 "차"를 substring 매칭해 6.5% 식품으로 분류)
