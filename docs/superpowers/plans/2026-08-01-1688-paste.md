...
이 저장소에는 `scripts/migrate-sourcing.mjs`가 있다 (`094_sourcing_shortlist.sql` 헤더에 `적용: node scripts/migrate-sourcing.mjs 094`로 명시). 일회용 스니펫을 쓰지 말고 이 스크립트를 쓴다.

**운영 DB다.** 적용은 소유자가 직접 한다 — 구현 에이전트는 DB에 접속하지 않는다:
...
- `patchShortlist`에 각 분기 추가. **붙여넣기에서 온 값(`buyKrwTotal`·`buyCnyTotal`·`orderQty1688`·`exchangeRate1688`)이 하나라도 오면 `pasted_at_1688 = now()`도 함께 세팅한다.**
  `intlShipPerUnit`은 제외한다 — 국제배송비는 붙여넣기가 아니라 사람이 따로 넣는 값이라, 이걸로 시각을 갱신하면 `pasted_at_1688`이 "붙여넣은 때"가 아니라 "마지막으로 뭔가 건드린 때"가 되어 환율 신선도를 판단할 수 없게 된다.
...
> **구현 중 정정 (2026-08-01).** 이 Task의 검증 목록에 구멍이 있었다.
> `buyCnyTotal`과 `exchangeRate1688`은 소수라 `Number.isInteger`를 못 걸어
> 계획에서 아예 빠져 있었고, 그대로 갔으면 `buyCnyTotal: "abc"`가 400이 아니라
> **numeric 컬럼에 직행해 500**이 났다. 음수나 자릿수가 밀린 환율도 조용히 저장돼
> 조회 시 계산되는 원가를 통째로 오염시켰을 것이다.
> `typeof === 'number' && Number.isFinite` + 범위 검사를 추가했다.
> `intlShipPerUnit`에도 상한을 뒀다 — 사람이 손으로 넣는 값이라 자릿수 실수가
> 가장 나기 쉬운 자리인데 혼자만 하한만 있었다.
...
function SupplierCompare({
  ...
  item: ShortlistItem;
  // Promise<void>가 아니라 Promise<boolean>이다. patchItem은 실패를 내부에서
  // 잡아 setError만 하고 정상 반환하므로, throw를 기대하고 catch를 걸면
  // 저장 실패에도 붙여넣은 원문이 지워진다 — 결제 화면을 다시 긁어와야 한다.
...
> **구현 중 정정 (2026-08-01).** 이 Task에서 계획서 오류 셋이 드러났다.
>
> 1. **`patchItem`은 throw하지 않는다.** 계획의 `save()`는 `try { await onPatch(...); setPaste('') } catch`
>    구조인데, 실제 `patchItem`은 실패를 내부에서 잡아 `setError`만 하고 정상 반환한다.
>    그대로 갔으면 **저장이 실패해도 붙여넣은 원문이 지워져** 결제 화면을 다시 긁어와야 했다.
>    `patchItem`을 `Promise<boolean>` 반환으로 바꾸고 성공했을 때만 지운다.
>    기존 호출부는 전부 `void patchItem(...)`이라 영향이 없다.
> 2. **비제어 입력 + `key` 리마운트 관례는 `DiscoveryTab.tsx`가 아니라 `ShortlistTab.tsx` 자신에 있다**
>    (쿠팡 실판가 input). `DiscoveryTab.tsx`에는 `defaultValue`가 한 군데도 없다.
> 3. **`ShortlistTab.tsx` 한 파일에 다 넣으면 ~800줄이 된다.** 비교 패널을
>    `SupplierCompare.tsx`로 분리했다 (572 + 462줄).
