...
| 16 | (최종 리뷰 I1·I3·I4) PC 영수증 옵션 · 실사 이전 구매 · RG 입고 완료 | ① **PC 영수증 모달**(`ReceiptIngestModal`)에는 나누기 화면을 두지 않는다 — 확정 전에 `sku-options`를 읽어 후보 0개·2개 이상인 줄(`linesNeedingPhone` = 나누기 전 `blockedLines` + 소분 추정 줄)이 있으면 확정을 막고 「옵션이 여러 개인 줄 N개는 휴대폰 영수증 화면에서 옵션별 수량을 나눠 확정합니다」 + `/m/receipt/<id>` 링크. 옵션 정보를 못 읽어도 막는다. 후보 1개뿐이면 전처럼 `{}`로 확정. 서버 실패 사유도 「휴대폰 영수증 화면(/m/receipt)」을 가리킨다 ② **실사 이전 구매**: `postReceiptLots`가 SKU를 모두 잠근 뒤, 되돌리지 않은 self `opening` 전표 시각 ≥ 구매일 KST 자정인 SKU는 원장 입고를 **건너뛰고** `skippedPreOpening`으로 돌려준다(`cost_entries`·품번 학습은 그대로). 확정 응답 `skipped_pre_opening: [{ line_no, sku_id, qty, name }]` → 휴대폰·PC 안내 「실사 이전 구매라 원장 입고는 건너뜀: <이름>」 ③ **RG 입고 완료**: `POST /api/erp/stock/rg-arrive` `{ items: [{ skuId, qty, requestId }] }` → `rg_inbound → rg` 이동(한 트랜잭션 · SKU 오름차순 잠금 · 멱등키 `rgdone:<uuid>` · 입고중 부족 409). `rgdone:`은 **되돌릴 수 있다**(#5 확장). 재고 화면은 RG 실재고 > 원장 RG이고 입고중 > 0인 행에 「반영」 옆 「입고 완료 m개 옮기기」(m = min(입고중, 실재고 − 원장 RG)) | ① PC에서 그대로 확정하면 그 줄만 실패하고 사유가 없는 화면을 가리켰다 ② 실사표는 그날 집에 있던 물건을 이미 셌다 — 그 전에 산 영수증을 뒤에 확정하면 같은 물건이 두 번 들어간다. 기준을 「기초 시각 ≥ 구매일 자정」으로 둔 것은 영수증에 구매일만 확실해서다(#10) — 같은 날 실사 뒤에 산 것도 건너뛰므로 그런 날은 화면에서 「지금 개수」로 맞춘다 ③ 1-C1에는 자동 입고 완료가 없다. RG에 들어간 물건을 「반영」(rg 지금 개수)으로만 맞추면 입고중이 남아 두 번 센다. `rgdone:`은 옛 원가 기록과 짝이 없는 원장만의 이동이라 #5의 되돌리기 제외 이유가 해당하지 않는다 |
...
Expected: 실패 수 ≤ 기준선(Task 0 Step 1) · tsc 0 · 자가시험 28행 전부 ✅(Task 4c에서 센 기록 2행 추가) · **기초(opening) 전표 0행 · 그 밖의 행은 실제 영수증 입고·RG 보내기만**(Task 8 배포 뒤 확정한 영수증은 `receipt`/`receipt_line`, RG 보내기는 `transfer`/`rg_shipment`로 이미 들어와 있을 수 있다 — 원장 0행을 기대하지 않는다) · 자가시험 SKU 0 · `ledger_cutover` 0.

```bash
node -e "
const fs=require('fs');const {Client}=require('pg');for(const l of fs.readFileSync('.env.local','utf8').split('\n')){const m=l.match(/^([A-Z_]+)=(.*)\$/);if(m)process.env[m[1]]=m[2].replace(/^[\"']|[\"']\$/g,'')}
(async()=>{const c=new Client({connectionString:process.env.SUPABASE_DB_URL,ssl:{rejectUnauthorized:false}});await c.connect();
console.log((await c.query(\"select kind, ref_type, count(*)::int n from erp.stock_ledger group by 1,2 order by 1,2\")).rows);
console.log((await c.query(\"select (select count(*) from erp.skus where key like 'selftest%')::int skus, (select count(*) from erp.sync_cursors where name='ledger_cutover')::int cursor\")).rows[0]);await c.end()})()"
```
Expected: 첫 줄에 `opening`이 없고 `receipt · receipt_line` / `transfer · rg_shipment`만(또는 빈 배열) · 둘째 줄 `{ skus: 0, cursor: 0 }`.

- [ ] **Step 2: 사용자에게 알리고 준비한다**

컨트롤러가 사용자에게 한 번에 전한다.
1. 🔵 **코드는 이미 배포돼 있다(Task 8).** 🔴 **게이트 절차:** ① **실사(세기) 전에 대기 중인 영수증을 정리한다** — 구매일을 못 읽은 줄 13개(확정 불가, 422)는 구매일을 채우거나 폐기한다. 정리한 영수증의 물건까지 세어야 실사가 맞는다 ② **Task 9는 배포 직후 바로 한다** ③ **배포와 게이트 사이에는 영수증을 확정하지 않고 RG 보내기도 하지 않는다**(최종 리뷰 I2) — 그 사이 원장 전표가 생긴 SKU는 실사표 불러오기에서 SKU 단위로 통째로 빠져(집·입고중·RG 모두) 화면에서 위치마다 「지금 개수」·RG 대조로 다시 입력해야 한다. 🔵 실사 이전 구매 영수증을 게이트 뒤에 확정해도 원장 입고는 건너뛴다(설계 해석 16 — 기초 시각 ≥ 구매일 KST 자정인 SKU. 화면에 「실사 이전 구매라 원장 입고는 건너뜀」)
...
🔴 **「반영」보다 「입고 완료 m개 옮기기」가 먼저다**(설계 해석 16 · I4): RG 실재고 > 원장 RG이고 RG입고중이 남은 행에는 그 버튼이 뜬다. 보낸 물건이 RG에 들어간 것이면 먼저 옮기고, 남은 차이만 「반영」한다 — 반영만 하면 입고중이 그대로 남아 같은 물건을 두 번 센다(일괄 반영 확인 창에도 입고중이 남은 SKU 수가 뜬다).
Expected: 다시 「RG 실재고 대조」를 누르면 **RG 불일치 0**.

그리고 **이전에 보낸 SKU의 RG입고중을 확인한다**(사용자): 실사표 `rg_inbound`로 불러온 SKU와 배포 뒤 RG 보내기로 보낸 SKU의 「RG입고중」 칸을 본다. Wing에서 입고 완료된 쉽먼트인데 입고중이 남아 있으면(RG 대조에서 이미 실재고에 잡혔는데 차이가 0이면 버튼이 뜨지 않는다) 그 칸을 「지금 개수」로 고친다(1-C1에는 자동 입고 완료가 없다 — 1-C2).
...
- 2026-09-26 게이트 입력값(사용자): 단가 — 파우치 핑크 1,562 · 쿨매트 블루구름 S 6,698 · 트루릴리젼 블랙 105(L) 24,990 · 니트 건조대 2단 1,691(위키 값 그대로) · **마크곤잘레스 키즈 2PK 26,990**(코스트코 692875 가격표 사진) · 핸드워시 750ml 6,495(옛 입고) · 퓨어틴 초코 12,995/팩. **퓨어틴 초코 실사 11 = 병** → 6개입 1팩 + 낱병 5 → **1팩만** 넣는다(낱병은 다음 입고와 합쳐 팩이 되면)
...
- 2026-09-26 적재(사용자 허용): `purchase-units-seed.ts --apply` 62행 + 693742 → 퓨어틴 커피 SKU 162 1행 = **purchase_units 63행**. `costco_item_map` 693742를 퓨어틴 커피(product_cost e7c57faa…)·**소분 입고(12개입 → 6개 팩)**로, 888450은 연결 해제·`default_decision=ask`. 🔵 693742가 「그대로 입고」였던 것이 퓨어틴 초코 박스 단위 중복 입고(1-B)의 원인과 같은 설정이었다
...
- **2026-09-26 기초재고 게이트 통과(Task 9).** 운영 앱 화면(`/erp/stock` 실사표 불러오기)으로 `opening-count-2026-09-26.csv` 적재 — 실사 시각 11:38 KST, 기준 시각(`ledger_cutover`) **2026-09-26 11:07:04 UTC**. 기초 전표 35건: 집 20건 225개 · RG 15건 230개 · 평가액 **4,084,898원**(집 2,432,362 · RG 1,652,536). 센 기록 215행. 입력 단가 7건은 「실행 기록」 참조. `rg-reconcile`: **원장 RG SKU 15 = 실재고 SKU 15 · 불일치 0** ✅ · 매핑 이슈 4(승인 목록 밖 vid 95812283106·95932746388·95833506834·95693450298, 합계 8개 — 원장 제외, 처리 결정 대기)
