...
**Goal:** `.claude/agents/coupang-report.md` 서브에이전트를 생성해, "보고서 생성해줘"라는 요청 한 마디로 쿠팡 Ads + Wing 데이터를 수집·분석하고 HTML/PDF 보고서를 자동 생성한다.

**Architecture:** Claude Code 서브에이전트(`.claude/agents/`)로 구현. gstack browse(`$B`)로 Ads·Wing 접속, 쿠키 유효 시 자동 로그인, 만료 시 handoff 폴백. 수집 데이터를 Opus 4.6으로 분석 후 인라인 HTML 템플릿에 채워 넣고, `$B pdf`로 PDF 변환 → 바탕화면 저장.

**Tech Stack:** Claude Opus 4.6, gstack browse (`~/.claude/skills/gstack/browse/dist/browse`), Bash
...
| 동작 | 파일 |
|---|---|
| **생성** | `.claude/agents/coupang-report.md` |
| **수정** | `CLAUDE.md` (라우팅 규칙 추가) |

---

## Task 1: 에이전트 파일 스켈레톤 생성

**Files:**
- Create: `.claude/agents/coupang-report.md`

- [ ] **Step 1: 에이전트 파일 생성**

`.claude/agents/coupang-report.md`를 다음 내용으로 생성한다:
...
```bash
head -5 .claude/agents/coupang-report.md
```
...
---
name: coupang-report
description: 쿠팡 광고(Ads) + Wing 데이터를...
```

- [ ] **Step 3: Commit**

```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): 쿠팡 보고서 에이전트 스켈레톤 생성"
```

---

## Task 2: 기간 파싱 + Ads 접속 로직 추가

**Files:**
- Modify: `.claude/agents/coupang-report.md`

- [ ] **Step 1: 에이전트 파일에 STEP 1-2 추가**

Task 1에서 생성한 파일의 "로그인 감지 함수" 섹션 뒤에 다음을 추가한다:
...
```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): 기간 파싱 + Ads 접속 로직 추가"
```

---

## Task 3: Ads 기간 필터 + 데이터 수집 로직 추가
...
```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): Ads 기간 필터 + 데이터 수집 로직 추가"
```

---

## Task 4: Wing 접속 + 데이터 수집 로직 추가
...
리뷰 페이지 경로가 다를 경우 Wing 좌측 메뉴에서 "리뷰 관리" 링크를 찾아 클릭:
```bash
$B links
# "리뷰" 포함 링크 찾기
$B goto <리뷰관리_URL>
...
```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): Wing 접속 + 데이터 수집 로직 추가"
```

---

## Task 5: Opus 분석 프롬프트 추가
...
```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): Opus 분석 프롬프트 추가"
```

---

## Task 6: HTML 보고서 템플릿 추가

**Files:**
- Modify: `.claude/agents/coupang-report.md`

- [ ] **Step 1: STEP 8 추가 (HTML 템플릿)**
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .funnel-bar { height: 44px; background: linear-gradient(90deg, #3b82f6, #1d4ed8); border-radius: 6px; display: flex; align-items: center; padding: 0 16px; color: white; font-weight: 700; font-size: 14px; transition: width 0.3s; }
  .funnel-label { min-width: 60px; font-size: 12px; color: #94a3b8; }
  .funnel-rate { font-size: 12px; color: #fbbf24; min-width: 80px; text-align: right; }
  .funnel-arrow { color: #475569; font-size: 18px; text-align: center; margin: 2px 0; }
...
| 플레이스홀더 | 치환값 |
|---|---|
| `{{PERIOD}}` | 분석 기간 (7 또는 30) |
| `{{TODAY}}` | `date +%Y년 %m월 %d일` 출력값 |
| `{{SELLER_NAME}}` | Wing에서 확인한 셀러명 |
| `{{IMPRESSIONS}}` | 노출수 (천단위 콤마 포함, 예: 183,969) |
| `{{CTR}}` | CTR 수치 (예: 0.19) |
| `{{CTR_STATUS}}` | danger/warning/good (0.5% 미만=danger, 0.8% 미만=warning, 이상=good) |
| `{{CVR}}` | CVR 수치 (예: 1.96) |
| `{{CVR_STATUS}}` | danger/warning/good (2% 미만=danger, 3% 미만=warning, 이상=good) |
| `{{ROAS}}` | ROAS 수치 |
| `{{ROAS_STATUS}}` | danger/warning/good (100% 미만=danger, 300% 미만=warning, 이상=good) |
| `{{SPEND}}` | 광고비 (천단위 콤마) |
| `{{CLICKS}}` | 클릭수 |
| `{{ORDERS}}` | 주문수 |
| `{{CTR_WIDTH}}` | CTR 시각화 너비 (CTR/1.5*100, 최대 100) |
| `{{CVR_WIDTH}}` | CVR 시각화 너비 (CVR/8*100, 최대 100) |
| `{{CTR_EVAL}}` | 업계 대비 평가 문구 (예: "업계 평균의 0.13배") |
| `{{CVR_EVAL}}` | 업계 대비 평가 문구 |
| `{{CAMPAIGN_ROWS}}` | 캠페인별 `<tr>` 행들 (상태에 따라 badge-on/badge-off) |
| `{{PRODUCT_ROWS}}` | 상품별 `<tr>` 행들 |
| `{{CAUSE_CARDS}}` | 원인 분석 `<div class="cause-card">` 2~4개 |
| `{{URGENT_ACTIONS}}` | `<li>` 목록 (3~5개) |
| `{{WEEK_ACTIONS}}` | `<li>` 목록 (3~5개) |
| `{{MONTH_ACTIONS}}` | `<li>` 목록 (3~5개) |
```
...
```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): HTML 보고서 템플릿 추가"
```

---

## Task 7: PDF 생성 + 완료 메시지 추가
...
HTML 파일이 생성된 것을 확인한 후 PDF로 변환한다:

```bash
B=~/.claude/skills/gstack/browse/dist/browse
HTML_PATH="/tmp/${DATE}_쿠팡_성과보고서.html"
PDF_PATH="/tmp/${DATE}_쿠팡_성과보고서.pdf"
DESKTOP_PATH="$HOME/Desktop/${DATE}_쿠팡_성과보고서.pdf"

$B pdf "$HTML_PATH" --format a4 --print-background --margins 0
cp "$PDF_PATH" "$DESKTOP_PATH"
```

`$B pdf`는 HTML 경로를 받아 동일 경로에 .pdf를 생성한다.
`$B pdf`가 실패하면 오류 메시지를 확인하고, 경로에 공백이 있는지 점검한다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━
📊 분석 기간: 최근 [PERIOD]일
📁 HTML: /tmp/[DATE]_쿠팡_성과보고서.html
📄 PDF:  ~/Desktop/[DATE]_쿠팡_성과보고서.pdf
━━━━━━━━━━━━━━━━━━━━━━━━━
핵심 요약:
- 노출: [IMPRESSIONS]회 | CTR: [CTR]% ([CTR평가])
- 주문: [ORDERS]건 | ROAS: [ROAS]%
- 최우선 액션: [긴급 액션 1순위]
```
```
...
```bash
git add .claude/agents/coupang-report.md
git commit -m "feat(agent): PDF 생성 + 완료 메시지 추가"
```

---

## Task 8: CLAUDE.md 라우팅 규칙 추가

**Files:**
- Modify: `CLAUDE.md`

- [ ] **Step 1: CLAUDE.md에 라우팅 규칙 추가**

`CLAUDE.md` 파일 끝에 다음을 추가한다:
...
```bash
git add CLAUDE.md
git commit -m "chore: coupang-report 에이전트 라우팅 규칙 추가"
```
...
**Files:**
- Read: `.claude/agents/coupang-report.md`

- [ ] **Step 1: 에이전트 파일 구조 검증**

```bash
# frontmatter 확인
head -6 .claude/agents/coupang-report.md

# 모든 STEP이 존재하는지 확인
grep "^## STEP" .claude/agents/coupang-report.md
```
...
- [ ] **Step 2: gstack browse 바이너리 존재 확인**

```bash
ls -la ~/.claude/skills/gstack/browse/dist/browse
```

Expected: `-rwxr-xr-x` 권한의 파일 존재. 없으면 에이전트 실행 전 gstack 재설치 필요.

- [ ] **Step 3: HTML 플레이스홀더 누락 검사**

```bash
grep "{{" .claude/agents/coupang-report.md | grep -v "치환값\|플레이스홀더\|예:" | head -20
```

모든 `{{...}}` 플레이스홀더가 템플릿 내부(Task 6 HTML 안)에만 있는지 확인. 템플릿 외부에 미치환 변수가 없어야 한다.

- [ ] **Step 4: 최종 파일 크기 확인**

```bash
wc -l .claude/agents/coupang-report.md
```

Expected: 300줄 이상 (템플릿 포함 시 충분한 분량).
...
에이전트가 `coupang-report` 서브에이전트로 디스패치되는지 확인. gstack browse 세션이 시작되고 로그인 감지가 실행되면 성공.
