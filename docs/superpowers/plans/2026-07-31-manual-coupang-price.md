...
**열을 새로 만들지 않는다.** 표에는 이미 `쿠팡 p25` 열이 있고(`ShortlistTab.tsx:362` 헤더, `:412-417` 셀) 값을 읽기 전용으로 보여준다. 그 셀 내용을 입력으로 바꾼다.

값 변경 시 `patchItem(itemNo, body)`를 호출한다 — 이미 있는 함수라 재검증·상태 갱신이 자동으로 따라온다:
...
헤더 문구는 `쿠팡 p25` → `쿠팡 실판가`로 바꾼다. 더 이상 분위수 추정치가 아니라 사람이 확인한 실판가다.

기존 셀에 붙어 있던 표본 수 접미사 `(n={it.coupangSampleN})`도 제거한다. Task 1이 `coupangSampleN`을 항상 `null`로 만들어 절대 렌더되지 않는 죽은 UI이고, 표본 개념 자체가 사라졌다(사람이 눈으로 본 값 1건).
