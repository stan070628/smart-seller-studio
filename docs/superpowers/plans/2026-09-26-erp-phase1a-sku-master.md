...
## 적재 결과 (2026-09-26, Task 7)

- 사용자 승인 후 `--apply`: SKU 216 · 리스팅 445(Wing 192·RG 102·네이버 104·토스 47) · 연결 455. 재실행 점검에서 전부 「동일」(멱등 확인).
- 검증: listing without sku 0 · **sync link unmapped 0**(품절 동기화 163행 전부 SKU로 연결) · recent sale vid unmapped 5 = 마스터버니 얼음주머니 RG 옵션 4개(7월 판매 8건, 현재 쿠팡 승인 목록에 없음 → 1-B에서 보관 SKU로 과거 판매 연결) + 네이버 주문번호 오인 1건(검증 SQL 오탐).

## 1-B·1-C로 넘기는 결정

| 항목 | 결정 |
|---|---|
| **당근마켓** (2026-09-26 사용자 승인) | API가 없어 리스팅 연결 대상이 아니다. 1-B에서 주문 채널에 `karrot`을 추가하고 **수동 판매 등록**(SKU·수량·판매가 → 자체보관 재고 차감 + 매출·원가 기록, `/m` 모바일 화면)을 만든다. 2단계에서 「당근에 올린 상품의 자체보관 재고 0」 알림 |
