...
CREATE TABLE IF NOT EXISTS public.discovery_logs (
  ...
  scanned_date    DATE         NOT NULL DEFAULT CURRENT_DATE,  -- 중복 방지 인덱스용 (IMMUTABLE 우회)
...
-- scanned_date 컬럼 사용 (표현식 인덱스의 IMMUTABLE 제약 우회)
CREATE UNIQUE INDEX IF NOT EXISTS uidx_discovery_daily_dedup
  ON public.discovery_logs (channel_source, product_id, scanned_date);
